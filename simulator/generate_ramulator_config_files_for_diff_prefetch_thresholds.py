...
import os
import errno
from batch_prefetcher_generator import get_hops_thresholds, get_count_thresholds, get_debug_flags, get_prefetcher_types, get_core_numbers, get_core_to_mem_config
...
mkdir_p(os.path.join(ramulator_config_dir, "pim"))

core_config = get_core_to_mem_config()
...
def generate_config_file(prefetcher_type, debug_flag, count_threshold, hops_threshold, adapative_flag, policy_name, core_number):
    ...
    output_dir = os.path.join(ramulator_config_dir, "prefetcher", "HMC-SubscriptionPF-"+prefetcher_type+"-"+debug_tag+"-"+policy_name+"-va"+str(core_number)+"-config.cfg")
