...
def extract_cycle(stat_file_location):
    ...
    return -1 if cycle_val == -1 or cycle_val_at_end_of_warmup == -1 else cycle_val - cycle_val_at_end_of_warmup
...
def extract_subscription_stats(stat_file_location, stat_name):
    ...
    with open(stat_file_location+".end_of_warmup", mode='r') as stat:
        for line in stat:
            if(line.find(stat_name)!= -1):
                value_at_end_of_warmup = int(line.split()[1])
    return -1 if value == -1 or value_at_end_of_warmup == -1 else value - value_at_end_of_warmup

def extract_per_vault_count(stat_file_location, label, core_number):
    ...
    with open(stat_file_location+".end_of_warmup", "r") as csv_file:
        csv_reader = csv.DictReader(csv_file, delimiter=' ')
        for row in csv_reader:
...
def extract_from_vault_count(stat_file_location, core_number):
    return extract_per_vault_count(stat_file_location, "CoreID", core_number)

def extract_to_vault_count(stat_file_location, core_number):
    return extract_per_vault_count(stat_file_location, "VaultID", core_number)
...
for suite in benchmark_suites_and_benchmarks_functions.keys():
    for benchmark_function in benchmark_suites_and_benchmarks_functions[suite]:
        ...
        baseline_stat_file_location = os.path.join(stats_folders, baseline_processor_type, core_number, full_benchmark_name+".ramulator.cycle_stats")
        baseline_cycle = -1
        ...
        if os.path.isfile(baseline_sub_stat_file_location) and os.path.isfile(baseline_sub_stat_file_location+".end_of_warmup"):
    ...
    for policy in plotting_prefetcher_policies:
        ...
        for suite in benchmark_suites_and_benchmarks_functions.keys():
            for benchmark_function in benchmark_suites_and_benchmarks_functions[suite]:
                ...
                full_benchmark_name = suite+"_"+benchmark_function
                benchmark_key = benchmark_suites_and_benchmarks_functions_to_short_name[suite][benchmark_function]
                stat_file_location = os.path.join(stats_folders, processor_type, policy+"_"+debug_tag, core_number, full_benchmark_name+".ramulator.cycle_stats")
                baseline_stat_file_location = os.path.join(stats_folders, baseline_processor_type, core_number, full_benchmark_name+".ramulator.cycle_stats")
                baseline_cycle = -1
                if os.path.isfile(baseline_stat_file_location):
                    baseline_cycle = extract_cycle(baseline_stat_file_location)
                ...
                normalized_cycles[benchmark_key] = 0 if baseline_cycle == -1 or cycle == -1 else float(baseline_cycle)/float(cycle)
        normalized_cycles_img_output_dir = os.path.join(output_dir,"normalized_cycles_"+prefetcher_type+"_"+policy+".png")
        normalized_cycles_csv_output_dir = normalized_cycles_img_output_dir.replace(".png", ".csv")
        normalized_cycles_x_lengend = "Workload"
        normalized_cycles_y_lengend = "Normalized Cycle"
        normalized_cycles_title = "Normalized Cycle by Workload"
        plt.legend()
        plt.bar(normalized_cycles.keys(), normalized_cycles.values(), color ='maroon',
                width = 0.5)
        plt.xticks(rotation=90)
        plt.xlabel(normalized_cycles_x_lengend)
        plt.ylabel(normalized_cycles_y_lengend)
        plt.title(normalized_cycles_title)
        plt.savefig(normalized_cycles_img_output_dir)
        plt.clf()
        with open(normalized_cycles_csv_output_dir, "w") as csv_file:
            csv_writer = csv.writer(csv_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            csv_writer.writerow([normalized_cycles_title])
            csv_writer.writerow([normalized_cycles_x_lengend, normalized_cycles_y_lengend])
            for key in normalized_cycles.keys():
                csv_writer.writerow([str(key).replace("\n", " "), str(normalized_cycles[key])])
plt.subplots_adjust(bottom=0.1)

# The following are benchmarks that impacted by our model
selected_benchmark_suites_and_benchmarks_functions = {"chai" : ["BS_BEZIER_KERNEL", "HSTO_HSTO", "OOPPAD_OOPPAD"],
    "darknet" : ["yolo_gemm_nn"],
    "hashjoin" : ["NPO_probehashtable", "PRH_histogramjoin"],
    "hpcg" : ["HPCG_ComputePrologation", "HPCG_ComputeRestriction", "HPCG_ComputeSPMV", "HPCG_ComputeSYMGS"],
    "ligra" : ["BC_edgeMapSparseUSAUserAdded", "BFSCC_edgeMapSparseUSAUserAdded", "BFS_edgeMapSparseUSAUserAdded", "PageRank_edgeMapDenseUSA",  "Triangle_edgeMapDenseRmat"],
    ...
    for hops_threshold in hops_thresholds:
        ...
        for suite in selected_benchmark_suites_and_benchmarks_functions.keys():
            for benchmark_function in selected_benchmark_suites_and_benchmarks_functions[suite]:
                ...
                if os.path.isfile(baseline_addr_dist_file_location) and os.path.isfile(baseline_addr_dist_file_location+".end_of_warmup"):
                    baseline_accesses_to_vaults = extract_to_vault_count(baseline_addr_dist_file_location, 32)
                ...
                for prefetcher_policy in prefetcher_policies:
                    processor_type = processor_type_prefix+prefetcher_type
                    stat_file_location = os.path.join(stats_folders, processor_type, prefetcher_policy+"_"+debug_tag, core_number, full_benchmark_name+".ramulator.cycle_stats")
                    ...
                    if not (os.path.isfile(sub_stat_file_location) and os.path.isfile(sub_stat_file_location+".end_of_warmup")):
                    ...
                    if not (os.path.isfile(addr_dist_file_location) and os.path.isfile(addr_dist_file_location+".end_of_warmup")):
                        cov_map[policy_name_to_label[prefetcher_policy]] = 0
                        cov_line.append(0)
                    else:
                        accesses_to_vaults = extract_to_vault_count(addr_dist_file_location, 32)
