...
output_filename = "stats_cycle_normalized.csv"
        ...
        for benchmark_function in benchmark_suites_and_benchmarks_functions[suite]:
            ...
            for core_number in core_numbers:
                ...
                for processor_type in processor_types:
                    ...
                    normalized_cycle = 0.0
                    if not cycle_val == 0:
                        if core_number == "1":
                            base_cycle[processor_type] = cycle_val
                            normalized_cycle = 1.0
                        else:
                            normalized_cycle = float(base_cycle[processor_type])/float(cycle_val)
                    current_line.append("N/A" if normalized_cycle == 0.0 else str(normalized_cycle))
