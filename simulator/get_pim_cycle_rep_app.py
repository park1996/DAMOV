...
processor_types = ["pim_ooo", "pim_ooo_netoh"]
core_number = "32"
csv_header = ["pim_ooo", "pim_ooo_netoh", "Difference"]

output_filename = "stats_cycle_pim_netoh.csv"
...
with open(output_filename, mode='w') as csv_file:
    ...
    for suite in benchmark_suites_and_benchmarks_functions.keys():
        for benchmark_function in benchmark_suites_and_benchmarks_functions[suite]:
            ...
            if current_line[0] != "N/A" and current_line[1] != "N/A":
                current_line.append(str(float(current_line[1])/float(current_line[0])))
            else:
                current_line.append("N/A")
